...
namespace Vanara.PInvoke.Tests
{
    ...
    public class CloudSyncCallbackArgs<T> : EventArgs where T : struct
    {
        ...
        public CloudSyncCallbackArgs(in CF_CALLBACK_INFO CallbackInfo, in CF_CALLBACK_PARAMETERS CallbackParameters)
        {
            ...
            info = CallbackInfo;
            SyncRootIdentity =
                new SafeHGlobalHandle(CallbackInfo.SyncRootIdentity, CallbackInfo.SyncRootIdentityLength, false);
        ...
        public CF_OPERATION_INFO MakeOpInfo(CF_OPERATION_TYPE opType, IntPtr syncStatus = default) =>
            new CF_OPERATION_INFO
            {
    ...
    public class PlaceHolderFileInfo : PlaceholderInfo
    {
        ...
        public static PlaceHolderFileInfo CreateFromFile(FileInfo fileInfo, bool inSync = true,
    ...
    internal class CloudSyncProvider : IDisposable
    {
        ...
        public CloudSyncProvider(string syncRootPath, string name, string iconResource = ",0", Version version = null,
            IEnumerable<(string name, int id)> customProperties = null)
        {
            if (name.Contains(" "))
            ...
            // PropertyDefinitions = customProperties?.Select(t => new StorageProviderItemPropertyDefinition
            //     { DisplayNameResource = t.name, Id = t.id }).ToList();
        ...
        public event EventHandler<CloudSyncCallbackArgs<CF_CALLBACK_PARAMETERS.DEHYDRATECOMPLETION>>
            NotifyDehydrateCompletion;

        public event EventHandler<CloudSyncCallbackArgs<CF_CALLBACK_PARAMETERS.DELETE>> NotifyDelete;

        public event EventHandler<CloudSyncCallbackArgs<CF_CALLBACK_PARAMETERS.DELETECOMPLETION>>
            NotifyDeleteCompletion;

        public event EventHandler<CloudSyncCallbackArgs<CF_CALLBACK_PARAMETERS.CLOSECOMPLETION>>
            NotifyFileCloseCompletion;

        public event EventHandler<CloudSyncCallbackArgs<CF_CALLBACK_PARAMETERS.OPENCOMPLETION>>
            NotifyFileOpenCompletion;

        public event EventHandler<CloudSyncCallbackArgs<CF_CALLBACK_PARAMETERS.RENAME>> NotifyRename;

        public event EventHandler<CloudSyncCallbackArgs<CF_CALLBACK_PARAMETERS.RENAMECOMPLETION>>
            NotifyRenameCompletion;
        ...
        {
            get => CfQuerySyncProviderStatus(key.Value, out var stat).Succeeded
                ? stat
                : CF_SYNC_PROVIDER_STATUS.CF_PROVIDER_STATUS_ERROR;
        ...
        public static bool IsSyncRoot(string syncRootPath)
        {
            ...
            return CfGetSyncRootInfoByPath(syncRootPath, CF_SYNC_ROOT_INFO_CLASS.CF_SYNC_ROOT_INFO_BASIC, mem, mem.Size,
                out var len).Succeeded;
        }
        ...
        /// </list>
        /// </remarks>
        public USN ConvertToPlaceholder(string relativeFilePath, bool inSync = true, bool dehydrate = false,
            IntPtr fileIdentity = default, uint fileIdentityLength = 0)
        {
            CfOpenFileWithOplock(relativeFilePath, CF_OPEN_FILE_FLAGS.CF_OPEN_FILE_FLAG_EXCLUSIVE, out var hCfFile)
                .ThrowIfFailed();
            using (hCfFile)
            {
        ...
        /// </list>
        /// </remarks>
        public USN ConvertToPlaceholder(HFILE fileHandle, bool inSync = true, bool dehydrate = false,
            IntPtr fileIdentity = default, uint fileIdentityLength = 0)
        {
            var flags = (inSync ? CF_CONVERT_FLAGS.CF_CONVERT_FLAG_MARK_IN_SYNC : 0) |
                        (dehydrate ? CF_CONVERT_FLAGS.CF_CONVERT_FLAG_DEHYDRATE : 0);
        ...
        public uint CreatePlaceholders(IList<PlaceholderInfo> placeholders)
        {
            ...
            for (int i = 0; i < placeholders.Count; i++)
            {
                ...
                var flags = ph is PlaceHolderFileInfo phf && phf.InSync
            ...
            CfCreatePlaceholders(SyncRootPath, entries, (uint)entries.Length, CF_CREATE_FLAGS.CF_CREATE_FLAG_NONE,
                out var done);
        ...
        public void ReportSyncStatus(string description, uint code)
        {
            ...
            var ss = new CF_SYNC_STATUS
                { StructSize = ssLen + descLen, Code = code, DescriptionLength = descLen, DescriptionOffset = ssLen };
            using var mem = new SafeCoTaskMemStruct<CF_SYNC_STATUS>(ss, ss.StructSize);
            StringHelper.Write(description, ((IntPtr)mem).Offset(Marshal.SizeOf<CF_SYNC_STATUS>()), out _, true,
                CharSet.Unicode, descLen);
            CfReportSyncStatus(SyncRootPath).ThrowIfFailed();
        }
        ...
        /// <param name="CallbackInfo">The callback information.</param>
        /// <param name="CallbackParameters">The callback parameters.</param>
        protected virtual void HandleEvent<T>(EventHandler<CloudSyncCallbackArgs<T>> handler,
            in CF_CALLBACK_INFO CallbackInfo, in CF_CALLBACK_PARAMETERS CallbackParameters) where T : struct
        {
        ...
        protected virtual void OnCancelFetchData(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(CancelFetchData, CallbackInfo, CallbackParameters);

        /// <summary>Callback to cancel a request for the contents of placeholder files.</summary>
        protected virtual void OnCancelFetchPlaceholders(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(CancelFetchPlaceholders, CallbackInfo, CallbackParameters);

        /// <summary>Callback to satisfy an I/O request, or a placeholder hydration request.</summary>
        protected virtual void OnFetchData(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) => HandleEvent(FetchData, CallbackInfo, CallbackParameters);

        /// <summary>Callback to request information about the contents of placeholder files.</summary>
        protected virtual void OnFetchPlaceholders(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(FetchPlaceholders, CallbackInfo, CallbackParameters);
        ...
        protected virtual void OnNotifyDehydrate(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(NotifyDehydrate, CallbackInfo, CallbackParameters);
        ...
        protected virtual void OnNotifyDehydrateCompletion(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(NotifyDehydrateCompletion, CallbackInfo, CallbackParameters);
        ...
        protected virtual void OnNotifyDelete(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(NotifyDelete, CallbackInfo, CallbackParameters);
        ...
        protected virtual void OnNotifyDeleteCompletion(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(NotifyDeleteCompletion, CallbackInfo, CallbackParameters);
        ...
        /// </summary>
        protected virtual void OnNotifyFileCloseCompletion(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(NotifyFileCloseCompletion, CallbackInfo, CallbackParameters);
        ...
        /// </summary>
        protected virtual void OnNotifyFileOpenCompletion(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(NotifyFileOpenCompletion, CallbackInfo, CallbackParameters);

        /// <summary>
        /// Callback to inform the sync provider that a placeholder under one of its sync roots is about to be renamed or moved.
        /// </summary>
        protected virtual void OnNotifyRename(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(NotifyRename, CallbackInfo, CallbackParameters);
        ...
        /// </summary>
        protected virtual void OnNotifyRenameCompletion(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(NotifyRenameCompletion, CallbackInfo, CallbackParameters);

        /// <summary>Callback to validate placeholder data.</summary>
        protected virtual void OnValidateData(in CF_CALLBACK_INFO CallbackInfo,
            in CF_CALLBACK_PARAMETERS CallbackParameters) =>
            HandleEvent(ValidateData, CallbackInfo, CallbackParameters);
        ...
        private async Task RegisterWithShell()
        {
            ...
            var syncRegistration = new CF_SYNC_REGISTRATION
            {
                ProviderId = Guid.NewGuid(),
                ProviderName = "Test",
                ProviderVersion = "Version.ToString()",
            };
            var syncPolicies = new CF_SYNC_POLICIES
            {
                ...
                InSync = CF_INSYNC_POLICY.CF_INSYNC_POLICY_TRACK_FILE_CREATION_TIME |
                         CF_INSYNC_POLICY.CF_INSYNC_POLICY_TRACK_DIRECTORY_CREATION_TIME,
                ...
                StructSize = UInt32.MaxValue,
            };
            CfRegisterSyncRoot(SyncRootPath, syncRegistration, syncPolicies, CF_REGISTER_FLAGS.CF_REGISTER_FLAG_DISABLE_ON_DEMAND_POPULATION_ON_ROOT);
